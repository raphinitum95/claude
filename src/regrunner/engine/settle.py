...
(() => {
  ...
  // lagSum: all the lateness added up (only beats more than 50 ms late: timer jitter is not a busy computer), for the time split (engine/timing.py).
  rr.lag = 0; rr.lagSum = 0;
  let beat = performance.now();
  setInterval(() => { const now = performance.now(); if (document.visibilityState !== 'hidden') { const late = now - beat - 100;
      rr.lag = Math.max(rr.lag, late); if (late > 50) rr.lagSum += late; } beat = now; }, 100);
  rr.takeLag = () => { const lag = rr.lag; rr.lag = 0; return lag; };
  rr.takeLagSum = () => { const sum = rr.lagSum; rr.lagSum = 0; return sum; };
})();
"""
